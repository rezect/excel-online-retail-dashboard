...
**Контекст данных:** построчные транзакции интернет‑магазина (InvoiceNo/StockCode/Quantity/UnitPrice/InvoiceDate/CustomerID/Country), где InvoiceNo, начинающийся с "C", означает отмену; UnitPrice в фунтах стерлингов.
...
- Лист `README`:
  - Описание датасета и ссылки на источник/лицензию.
  - Бизнес-вопросы (пример): динамика выручки, топ‑страны, топ‑товары, удержание/частота покупок, доля отмен.
- Лист `Metrics Dictionary`:
  - Определения метрик (Revenue, Orders, AOV, Cancel Rate и т. п.) и правила расчёта, включая трактовку отмен через InvoiceNo="C…".
...
### Acceptance criteria
- Любой человек, открыв файл, понимает: откуда данные, что считается, какие ограничения.
...
### Power Query задачи
- Создать запрос `stg_sales_raw` (источник: Excel‑файл датасета).
- Привести типы:
  - `InvoiceDate` → datetime; `Quantity`, `UnitPrice` → number; `CustomerID` → (часто лучше text/whole number по ситуации).
- Добавить столбцы:
  - `IsCancellation` = InvoiceNo начинается с "C" (правило датасета).
  - `LineRevenue` = Quantity * UnitPrice.
- Минимальные проверки качества (лист `Data_Quality` как таблица/сводка):
  - Кол-во строк, % пропусков CustomerID, диапазон дат, наличие отрицательных Quantity/UnitPrice (зафиксировать как наблюдение/правило).

### Acceptance criteria
- `Refresh All` обновляет staging без ручных правок.
- Типы и вычисляемые поля воспроизводимы при каждом обновлении.
...
- `dim_Product`: DISTINCT по `StockCode`, `Description`.
- `dim_Customer`: DISTINCT по `CustomerID`, `Country`.
- `dim_Country` (опционально): DISTINCT по `Country`.
- `dim_Date`: календарь (можно генерировать в PQ) на диапазон `InvoiceDate`.
...
- `m_CancelledOrders` и/или `m_CancelRate` (используя `IsCancellation` из правила InvoiceNo="C").
- `m_ItemsSold` = SUM(Quantity) (отдельно обдумать: включать ли отмены).

### Acceptance criteria
- Все KPI пересчитываются при фильтрах (slicers) по Country/Date/Product.
...
- Проверить корректность на границах месяцев/года (особенно если время в InvoiceDate).

### Acceptance criteria
- На графике по месяцам/неделям корректно считается выручка и её MTD/YTD без ручных диапазонов.
...
### Состав дашборда
- KPI карточки: Revenue, Orders, AOV, Customers, Cancel Rate.
- Графики:
  - Revenue по времени (месяц/неделя).
  - Top‑10 Countries по Revenue.
  - Top‑10 Products по Revenue.
- Срезы (slicers): Country, Year/Month (из `dim_Date`).

### Требования к оформлению
- Единые подписи и единицы (валюта — фунты стерлингов из описания датасета).
...
### Acceptance criteria
- Сегменты пересчитываются при фильтрах (например, по Country) и после обновления данных.
...
- “Проблемность”:
  - Доля отмен по товару (используя `IsCancellation` из правила InvoiceNo="C").
- Визуализации: таблица A/B/C, топ отменяемых товаров.

### Acceptance criteria
- ABC обновляется динамически при фильтрах и после Refresh (без ручной сортировки).
...
- Лист `Data_Quality` с метриками:
  - % строк без CustomerID; количество отмен (по InvoiceNo="C") и их доля; отрицательные Quantity/UnitPrice.
- Решения и допущения (в `README`):
  - Считать ли отрицательные Quantity как возвраты; исключать ли строки с нулевой/отрицательной ценой; как считать Orders при отменах.
- (опционально) “слои” факта:
  - `fct_Sales_All` и `fct_Sales_Net` (net = без отмен/под выбранную бизнес-логику).

### Acceptance criteria
- Любой спорный момент имеет явное правило и отражён в документации, а не “скрыт” в формулах.
...
### Чек-лист
- `README` содержит:
  - бизнес-цели, датасет/ссылки, словарь метрик, принятые допущения, список листов и как пользоваться дашбордом.
- “Технический паспорт”:
...
### Acceptance criteria
- Файл можно передать другому человеку вместе с исходным `Online Retail.xlsx`, и он сможет обновить и пользоваться отчётом без объяснений “на словах”.
